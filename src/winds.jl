...
const fan_ports = ["usb-Arduino__www.arduino.cc__0043_95735353032351F0F0F0-if00"]
nports = length(fan_ports)
const rpmplt_cont = (colors = repeat(1:nports, inner = [3]), x = vcat(((i - 1)*(nports - 1) + 1 : (nports - 1)*i - 1 for i in 1:nports)...), y = top_rpm*ones(3nports), resolution = (540, round(Int, 3nports + 3*540/(3nports+4))))
